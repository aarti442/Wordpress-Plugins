<?php
/** 
 * The file used to manage list page style 1.
 * 
...
?>
<!-- listing-1 style-1 Start -->
<div class="listing-1 <?php echo $cls; ?>">
    <!-- listing-style-1 Start -->
    <div class="listing-block listing-style-1">
            ...
            <div class="bcp-flex bcp-justify-content-between">
                <?php if ($view == '') { ?>
                    ...
                    <div class="bcp-form-group listing-search">
                        ...
                        ?>
                        <input class="module_search_name <?php echo $class; ?>" module-name="<?php echo $module_name; ?>" placeholder="<?php _e('Search', 'bcp_portal'); ?>" value="<?php echo (isset($search) && $search != '') ? $search : ''; ?>"><a href="JavaScript:void(0);" id="btn_clear" module-name="<?php echo $module_name; ?>" title="<?php _e("Clear", 'bcp_portal'); ?>"><span class="lnr lnr-cross clear-btn"></span></a>
                ...
                <table class="bcp-table" id="bcp_module_table">
                    <thead>
                        <tr>
                            ...
                            foreach ($list_records['layout'] as $key => $value) {
                                ...
                                if ($label_value == 'Closed' || $label_value == 'FileExtension' || $label_value == 'ContentType') {
                                    continue;
                                }
                                ?>
                                <th>
                                    <a href="javascript:void(0);" class="bcp-action " module-name="<?php echo $module_name; ?>" module-action="<?php echo $view1; ?>" order-by="<?php echo $key . ' ' . $orders; ?>">
                    ...
                    <tbody>
                        ...
                        foreach ($list_records['records'] as $k => $records) { 
                            ...
                            $ContentType = isset($records['ContentType']) ? $records['ContentType'] : "";
                            if ($FileExtension) {
                                $FileExtension = $records['FileExtension'];
                                unset($records['FileExtension']);
                            }
                            if ($ContentType) {
                                $FileExtension = $records['ContentType'];
                                unset($records['ContentType']);
                            }
                            ...
                            <tr>
                                ...
                                <?php if (!empty($list_records['Action'])) { ?>
                                    ...
                                    if ($module_name == 'contentdocument') {
                                        $class_add = 'scp_contentdocument_action';
                                    }
                                    ?>
                                    <td class="dcp_hover_td listing-action text-center <?php echo $class_add; ?>">
                                        ...
                                        foreach ($list_records['Action'] as $action) { 
                                            ...
                                            <?php } else if ($action == 'solution') { $actionFlag = 1;?>
                                                <a class="solutions-action bcp-action" href="<?php echo $solution_url; ?>" title="<?php _e("Solutions", 'bcp_portal'); ?>">
                                                    ...
                                                    <?php _e('Solutions', 'bcp_portal'); ?>
                                                </a>
                                            <?php } else if (($action == 'edit' && $module_name != 'case' && (($RecordCreatedByContact != "" && $RecordCreatedByContact == $_SESSION['bcp_contact_id'])|| $module_name == 'account')) || ($action == 'edit' && $IsClosed != 1 && $module_name == 'case'&& ( ($RecordCreatedByContact != "" && $RecordCreatedByContact == $_SESSION['bcp_contact_id']) || $module_name == 'account'))) { 
                                                $actionFlag = 1; ?>
                                                <a class="edit-action bcp-action" href="<?php echo $edit_url; ?>" title="<?php _e("Edit", 'bcp_portal'); ?>"><span class="lnr lnr-pencil"></span><?php _e('Edit', 'bcp_portal'); ?></a>
                                                <?php
                                            }
                                            if ($action == 'download') {
                                                $actionFlag = 1;
    ...
    if ($list_records != null) {
        ...
        echo $action_class->bcp_pagination($total_records, $limit, $paged, $module_name, $action, $order_by, $search, $style = 'style_1');
    }
    ?>
</div>
<!-- listing-1 style-1 End -->